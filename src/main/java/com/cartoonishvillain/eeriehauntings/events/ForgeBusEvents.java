...
import com.cartoonishvillain.eeriehauntings.networking.ClientSoundPacket;
            ...
            event.player.getCapability(PlayerCapability.INSTANCE).ifPresent(h->{
                if(h.getIsHaunted()) {
                    if (event.player.level.isDay()) {
                        int soundID = event.player.getRandom().nextInt(EerieHauntings.lowEndSounds.size());
                        ClientSoundMessenger.sendTo(new ClientSoundPacket(event.player.getId(), soundID), event.player);
