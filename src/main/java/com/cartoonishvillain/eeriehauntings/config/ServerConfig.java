...
public class ServerConfig {
    ...
    public ServerConfig(ForgeConfigSpec.Builder builder, ConfigHelper.Subscriber subscriber){
        ...
        this.NORMALHAUNTCHANCE = subscriber.subscribe(builder.comment("The percent chance a player is to be haunted on a given night by default. If the player isn't haunted at night, this is the value they will start off on for the next night.").defineInRange("defaultHauntChance", 5.0, 0.0, 100));        this.VILLAGERHAUNTCHANCE = subscriber.subscribe(builder.comment("The percentage increase in haunt chance a player experiences when they kill a villager.").defineInRange("villagerHauntChance", 10.0, 0, 100));
        ...
        this.MISCHAUNTCHANCE = subscriber.subscribe(builder.comment("The percentage increase in haunt chance a player experiences when they kill anything outside of the other categories.").defineInRange("miscHauntChance", 1.5, 0, 100));        this.MEDIUMEFFECT = subscriber.subscribe(builder.comment("Enables or disables medium strength events (blindness, slowness, weakness) (replaces with weak sound events)").define("mediumStrengthEvents", true));
